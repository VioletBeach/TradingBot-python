    ...
    try :
        ...
        if repeat_count % 1800 == 0 :
            ...
            if repeat_count == 21600 :
                for i in coins :
                    history = pyupbit.get_ohlcv(i['key'], interval="minute1", count=nday)['high']
                    history_high = float(history[0])
                    for a in history :
                        if history_high > float(a) :
                            history_high = float(a)
                    history_high=round_price(history_high*0.9765)
                ...
                for i in coins :
                    if "uuid" in i['sell'] :
                        print(i['key'],'\tcancel',upbit.cancel_order(i['sell']['uuid']),'\n')
