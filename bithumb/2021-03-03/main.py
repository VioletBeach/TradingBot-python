...
while(1) :
    try :
        ...
        for i in coins :
            ...
            if(i['order'][0]!=0 and bids_price>=i['inputs']['buys_price']*1.010) :
                try :
                    print(i['inputs']['key'],'\t',bithumb.cancel_order(i['order'][0]),'cancel, priceNum:1','\t'+now)
                finally:
                    i['order'][0]=0
            if(i['order'][1]!=0 and bids_price>=i['inputs']['buys_price']*1.006) :
                try :
                    print(i['inputs']['key'],'\t',bithumb.cancel_order(i['order'][1]),'cancel, priceNum:2','\t'+now)
                finally:
                    i['order'][1]=0
            if(i['order'][2]!=0 and bids_price>=i['inputs']['buys_price']*1.003) :
                try :
