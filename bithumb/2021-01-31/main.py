...
while(1) :
    try :
        ...
        if(order_soda!=0 and soda_price>=buy_soda_price*1.012) :
             ...
             is_buy_soda=0
        if(is_sell_soda==1) :
            if(all['data']['SOC']['asks'][0]['price']>buy_soda_price*yeild) :
                is_sell_soda=0
                is_order_soda=0
        ...
        if(order_con!=0 and con_price>=buy_con_price*1.012) :
            ...
            is_buy_con=0
        if(is_sell_con==1) :
            if(all['data']['CON']['asks'][0]['price']>buy_con_price*yeild) :
                is_sell_con=0
                is_order_con=0
        ...
        if(order_lamba!=0 and lamba_price>=buy_lamba_price*1.012) :
            ...
            is_buy_lamba=0
        if(is_sell_lamba==1) :
            if(all['data']['LAMB']['asks'][0]['price']>buy_lamba_price*yeild) :
                is_sell_lamba=0
                is_order_lamba=0
        ...
        if(order_eforce!=0 and eforce_price>=buy_eforce_price*1.012) :
            ...
            is_buy_eforce=0
        if(is_sell_eforce==1) :
            if(all['data']['WOZX']['asks'][0]['price']>buy_eforce_price*yeild) :
                is_sell_eforce=0
                is_order_eforce=0
        ...
        if(order_queenB!=0 and queenB_price>=buy_queenB_price*1.012) :
            ...
            is_buy_queenB=0
        if(is_sell_queenB==1) :
            if(all['data']['QBZ']['asks'][0]['price']>buy_queenB_price*yeild) :
                is_sell_queenB=0
                is_order_queenB=0
        ...
        if(order_ringX!=0 and ringX_price>=buy_ringX_price*1.012) :
            ...
            is_buy_ringX=0
        if(is_sell_ringX==1) :
            if(all['data']['RINGX']['asks'][0]['price']>buy_ringX_price*yeild) :
                is_sell_ringX=0
                is_order_ringX=0
        ...
        if(order_proton!=0 and proton_price>=buy_proton_price*1.012) :
            ...
            is_buy_proton=0
        if(is_sell_proton==1) :
            if(all['data']['XPR']['asks'][0]['price']>buy_proton_price*yeild) :
                is_sell_proton=0
                is_order_proton=0
        ...
        if(order_ae!=0 and ae_price>=buy_ae_price*1.012) :
            ...
            is_buy_ae=0
        if(is_sell_ae==1) :
            if(all['data']['AE']['asks'][0]['price']>buy_ae_price*yeild) :
                is_sell_ae=0
                is_order_ae=0
        ...
        if(order_gom2!=0 and gom2_price>=buy_gom2_price*1.012) :
            ...
            is_buy_gom2=0
        if(is_sell_gom2==1) :
            if(all['data']['GOM2']['asks'][0]['price']>buy_gom2_price*yeild) :
                is_sell_gom2=0
                is_order_gom2=0
        ...
        if(order_vsys!=0 and vsys_price>=buy_vsys_price*1.012) :
            ...
            is_buy_vsys=0
        if(is_sell_vsys==1) :
            if(all['data']['VSYS']['asks'][0]['price']>buy_vsys_price*yeild) :
                is_sell_vsys=0
                is_order_vsys=0
        ...
        if(order_cennz!=0 and cennz_price>=buy_cennz_price*1.012) :
            ...
            is_buy_cennz=0
        if(is_sell_cennz==1) :
            if(all['data']['CENNZ']['asks'][0]['price']>buy_cennz_price*yeild) :
                is_sell_cennz=0
                is_order_cennz=0
        ...
        if(order_mbl!=0 and mbl_price>=buy_mbl_price*1.012) :
            ...
            is_buy_mbl=0
        if(is_sell_mbl==1) :
            if(all['data']['MBL']['asks'][0]['price']>buy_mbl_price*yeild) :
                is_sell_mbl=0
                is_order_mbl=0
        ...
        if(order_eos!=0 and eos_price>=buy_eos_price*1.012) :
             ...
             is_buy_eos=0
        if(is_sell_eos==1) :
            if(all['data']['EOS']['asks'][0]['price']>buy_eos_price*yeild) :
                is_sell_eos=0
                is_order_eos=0
        ...
        if(order_ipx!=0 and ipx_price>=buy_ipx_price*1.012) :
             ...
             is_buy_ipx=0
        if(is_sell_ipx==1) :
            if(all['data']['IPX']['asks'][0]['price']>buy_ipx_price*yeild) :
                is_sell_ipx=0
                is_order_ipx=0
        ...
        if(order_fleta!=0 and fleta_price>=buy_fleta_price*1.012) :
             ...
             is_buy_fleta=0
        if(is_sell_fleta==1) :
            if(all['data']['FLETA']['asks'][0]['price']>buy_fleta_price*yeild) :
                is_sell_fleta=0
                is_order_fleta=0
        ...
        if(order_wemix!=0 and wemix_price>=buy_wemix_price*1.012) :
            ...
            is_buy_wemix=0
        if(is_sell_wemix==1) :
            if(all['data']['WEMIX']['asks'][0]['price']>buy_wemix_price*yeild) :
                is_sell_wemix=0
                is_order_wemix=0
        ...
        if(is_buy_tmtg==1) :
            ...
            is_sell_tmtg=1
        if(is_sell_tmtg==1) :
            if(all['data']['TMTG']['asks'][0]['price']>buy_tmtg_price*yeild) :
                is_sell_tmtg=0
                is_order_tmtg=0
        ...
        if(is_buy_em==1) :
            ...
            is_sell_em=1
        if(is_sell_em==1) :
            if(all['data']['EM']['asks'][0]['price']>buy_em_price*yeild) :
                is_sell_em=0
                is_order_em=0
        ...
        if(is_buy_evz==1) :
            ...
            is_sell_evz=1
        if(is_sell_evz==1) :
            if(all['data']['EVZ']['asks'][0]['price']>buy_evz_price*yeild) :
                is_sell_evz=0
                is_order_evz=0
        ...
        if(is_buy_aoa==1) :
            ...
            is_sell_aoa=1
        if(is_sell_aoa==1) :
            if(all['data']['AOA']['asks'][0]['price']>buy_aoa_price*yeild) :
                is_sell_aoa=0
                is_order_aoa=0
        ...
        if(is_buy_adp==1) :
            ...
            is_sell_adp=1
        if(is_sell_adp==1) :
            if(all['data']['ADP']['asks'][0]['price']>buy_adp_price*yeild) :
                is_sell_adp=0
                is_order_adp=0

        gxc_price = all['data']['GXC']['bids'][0]['price']
        gxc_price=float(gxc_price)
        print("지엑스체인의 현재 가격 : ", gxc_price)
        if(order_gxc==0 and gxc_price<=buy_gxc_price*1.003) :
        ...
        if(is_buy_gxc==1) :
            ...
            is_sell_gxc=1
        if(is_sell_gxc==1) :
            if(all['data']['GXC']['asks'][0]['price']>buy_gxc_price*yeild) :
                is_sell_gxc=0
                is_order_gxc=0
            
        xsr_price = all['data']['XSR']['bids'][0]['price']
        xsr_price=float(xsr_price)
        print("젠서의 현재 가격 : ", xsr_price)
        if(order_xsr==0 and xsr_price<=buy_xsr_price*1.003) :
        ...
        if(is_buy_xsr==1) :
            ...
            is_sell_xsr=1
        if(is_sell_xsr==1) :
            if(all['data']['XSR']['asks'][0]['price']>buy_xsr_price*yeild) :
                is_sell_xsr=0
                is_order_xsr=0
        ...
        print("메타디움의 현재 가격 : ", meta_price)
        if(order_meta==0 and meta_price<=buy_meta_price*1.003) :
        ...
        if(is_buy_meta==1) :
            ...
            is_sell_meta=1
        if(is_sell_meta==1) :
            if(all['data']['META']['asks'][0]['price']>buy_meta_price*yeild) :
                is_sell_meta=0
                is_order_meta=0

        apm_price = all['data']['APM']['bids'][0]['price']
        apm_price=float(apm_price)
        print("에이피엠의 현재 가격 : ", apm_price)
        if(order_apm==0 and apm_price<=buy_apm_price*1.003) :
        ...
        if(is_buy_apm==1) :
            ...
            is_sell_apm=1
        if(is_sell_apm==1) :
            if(all['data']['APM']['asks'][0]['price']>buy_apm_price*yeild) :
                is_sell_apm=0
                is_order_apm=0

        mix_price = all['data']['MIX']['bids'][0]['price']
        mix_price=float(mix_price)
        print("믹스마블의 현재 가격 : ", mix_price)
        if(order_mix==0 and mix_price<=buy_mix_price*1.003) :
        ...
        if(is_buy_mix==1) :
            ...
            is_sell_mix=1
        if(is_sell_mix==1) :
            if(all['data']['MIX']['asks'][0]['price']>buy_mix_price*yeild) :
                is_sell_mix=0
                is_order_mix=0

        obsr_price = all['data']['OBSR']['bids'][0]['price']
        obsr_price=float(obsr_price)
        print("옵저버의 현재 가격 : ", obsr_price)
        if(order_obsr==0 and obsr_price<=buy_obsr_price*1.003) :
        ...
        if(is_buy_obsr==1) :
            ...
            is_sell_obsr=1
        if(is_sell_obsr==1) :
            if(all['data']['OBSR']['asks'][0]['price']>buy_obsr_price*yeild) :
                is_sell_obsr=0
                is_order_obsr=0

        dvc_price = all['data']['DVC']['bids'][0]['price']
        dvc_price=float(dvc_price)
        print("드래곤베인의 현재 가격 : ", dvc_price)
        if(order_dvc==0 and dvc_price<=buy_dvc_price*1.003) :
        ...
        if(is_buy_dvc==1) :
            ...
            is_sell_dvc=1
        if(is_sell_dvc==1) :
            if(all['data']['DVC']['asks'][0]['price']>buy_dvc_price*yeild) :
                is_sell_dvc=0
                is_order_dvc=0

        aion_price = all['data']['AION']['bids'][0]['price']
        aion_price=float(aion_price)
        print("아이온의 현재 가격 : ", aion_price)
        if(order_aion==0 and aion_price<=buy_aion_price*1.003) :
        ...
        if(is_buy_aion==1) :
            ...
            is_sell_aion=1
        if(is_sell_aion==1) :
            if(all['data']['AION']['asks'][0]['price']>buy_aion_price*yeild) :
                is_sell_aion=0
                is_order_aion=0

        mlk_price = all['data']['MLK']['bids'][0]['price']
        mlk_price=float(mlk_price)
        print("밀크의 현재 가격 : ", mlk_price)
        if(order_mlk==0 and mlk_price<=buy_mlk_price*1.003) :
        ...
        if(is_buy_mlk==1) :
            ...
            is_sell_mlk=1
        if(is_sell_mlk==1) :
            if(all['data']['MLK']['asks'][0]['price']>buy_mlk_price*yeild) :
                is_sell_mlk=0
                is_order_mlk=0

        basic_price = all['data']['BASIC']['bids'][0]['price']
        basic_price=float(basic_price)
        print("베이직의 현재 가격 : ", basic_price)
        if(order_basic==0 and basic_price<=buy_basic_price*1.003) :
        ...
        if(is_buy_basic==1) :
            ...
            is_sell_basic=1
        if(is_sell_basic==1) :
            if(all['data']['BASIC']['asks'][0]['price']>buy_basic_price*yeild) :
                is_sell_basic=0
                is_order_basic=0

        egg_price = all['data']['EGG']['bids'][0]['price']
        egg_price=float(egg_price)
        print("네스트리의 현재 가격 : ", egg_price)
        if(order_egg==0 and egg_price<=buy_egg_price*1.003) :
        ...
        if(is_buy_egg==1) :
            ...
            is_sell_egg=1
        if(is_sell_egg==1) :
            if(all['data']['EGG']['asks'][0]['price']>buy_egg_price*yeild) :
                is_sell_egg=0
                is_order_egg=0
            
        rnt_price = all['data']['RNT']['bids'][0]['price']
        rnt_price=float(rnt_price)
        print("원루트 네트워크의 현재 가격 : ", rnt_price)
        if(order_rnt==0 and rnt_price<=buy_rnt_price*1.003) :
        ...
        if(is_buy_rnt==1) :
            ...
            is_sell_rnt=1
        if(is_sell_rnt==1) :
            if(all['data']['RNT']['asks'][0]['price']>buy_rnt_price*yeild) :
                is_sell_rnt=0
                is_order_rnt=0

        dad_price = all['data']['DAD']['bids'][0]['price']
        dad_price=float(dad_price)
        print("다드의 현재 가격 : ", dad_price)
        if(order_dad==0 and dad_price<=buy_dad_price*1.003) :
        ...
        if(is_buy_dad==1) :
            ...
            is_sell_dad=1
        if(is_sell_dad==1) :
            if(all['data']['DAD']['asks'][0]['price']>buy_dad_price*yeild) :
                is_sell_dad=0
                is_order_dad=0

        wet_price = all['data']['WET']['bids'][0]['price']
        wet_price=float(wet_price)
        print("위쇼토큰의 현재 가격 : ", wet_price)
        if(order_wet==0 and wet_price<=buy_wet_price*1.003) :
        ...
        if(is_buy_wet==1) :
            ...
            is_sell_wet=1
        if(is_sell_wet==1) :
            if(all['data']['WET']['asks'][0]['price']>buy_wet_price*yeild) :
                is_sell_wet=0
                is_order_wet=0

        apix_price = all['data']['APIX']['bids'][0]['price']
        apix_price=float(apix_price)
        print("아픽스의 현재 가격 : ", apix_price)
        if(order_apix==0 and apix_price<=buy_apix_price*1.003) :
        ...
        if(is_buy_apix==1) :
            ...
            is_sell_apix=1
        if(is_sell_apix==1) :
            if(all['data']['APIX']['asks'][0]['price']>buy_apix_price*yeild) :
                is_sell_apix=0
                is_order_apix=0

        true_price = all['data']['TRUE']['bids'][0]['price']
        true_price=float(true_price)
        print("트루체인의 현재 가격 : ", true_price)
        if(order_true==0 and true_price<=buy_true_price*1.003) :
        ...
        if(is_buy_true==1) :
            ...
            is_sell_true=1
        if(is_sell_true==1) :
            if(all['data']['TRUE']['asks'][0]['price']>buy_true_price*yeild) :
                is_sell_true=0
                is_order_true=0
